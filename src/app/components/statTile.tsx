...
const StateTile: React.FC<IStat> = (stat) => {
  return (
    <div className="border-2 border-gray-200 rounded-md flex-1 p-2 md:p-4">
      ...
      <div className="flex items-center gap-2 md:gap-4 pt-2 md:pt-4">
        ...
        <div className={`text-[10px] md:text-[12px] font-semibold w-fit p-2 py-1 rounded-full ${stat.changed === "increased" ? "text-green-500 bg-green-200" : "text-red-500  bg-red-200"} flex items-center gap-1`}>
          <span>
            {stat.changed === "increased" ? "+" : "-"} {stat.changedBy}
          </span>
          <span>{stat.changed === "increased" ? <TrendingUp size={16} /> : <TrendingDown size={16}/>}</span>
        </div>
      </div>
    </div>
  );
